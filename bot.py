...
try:
    ...
    buttons_col = db['buttons']  # New: Custom buttons collection
...
async def handle_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str):
    ...
    # Delete previous messages for the same video from this user
    if user.id in user_video_messages:
        if video_id in user_video_messages[user.id]:
            for old_msg_id in user_video_messages[user.id][video_id]:
                try:
                    await context.bot.delete_message(chat_id=chat_id, message_id=old_msg_id)
                    logger.info(f"🗑️ Deleted old message {old_msg_id} for video {video_id}")
                except Exception as e:
                    logger.debug(f"Could not delete message {old_msg_id}: {e}")
            # Clear the list
            user_video_messages[user.id][video_id] = []
    else:
        user_video_messages[user.id] = {}
    ...
    if not_joined:
        ...
        # Track this message for deletion if requested again
        if video_id not in user_video_messages[user.id]:
            user_video_messages[user.id][video_id] = []
        user_video_messages[user.id][video_id].append(msg.message_id)
    ...
    try:
        ...
        # Track these messages for deletion if requested again
        if video_id not in user_video_messages[user.id]:
            user_video_messages[user.id][video_id] = []
        user_video_messages[user.id][video_id].extend([video_msg.message_id, after_msg.message_id])
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("verify_"):
        ...
        for channel in force_channels:
            try:
                ...
                if member.status in ['left', 'kicked', 'restricted']:
                    ...
                    logger.info(f"Verify: ✅ User {user.id} confirmed in {channel['username']}: {member.status}")
        ...
        if not_joined:
            ...
            channel_names = ", ".join([f"@{ch['username']}" for ch in not_joined])
            await query.answer(
        ...
        try:
            ...
            # Track for future deletion
            if user.id not in user_video_messages:
                user_video_messages[user.id] = {}
            if video_id not in user_video_messages[user.id]:
                user_video_messages[user.id][video_id] = []
            user_video_messages[user.id][video_id].extend([video_msg.message_id, after_msg.message_id])
    ...
    if data.startswith("copy_"):
        ...
        await query.answer(f"✅ Shortcode copied: {shortcode}", show_alert=True)
        return
...
async def channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Create admin notification with copy button
...
👆 Click the shortcode above to copy!
...
Users can now watch this video!"""
        ...
        # Add inline button for alternative copy method
        keyboard = [[InlineKeyboardButton("📋 Copy Shortcode", callback_data=f"copy_{message_id}")]]
