        ...
        if video_id in user_video_messages[user.id]:
            for old_msg_id in user_video_messages[user.id][video_id]:
                try:
                    ...
                    logger.error(f"Could not delete message {old_msg_id}: {e}")
            # Clear the list
    ...
    if data.startswith("verify_"):
        ...
        for channel in force_channels:
            try:
                ...
                logger.error(f"Error checking membership for {user.id} in {channel['channel_id']}: {e}")
                # If error checking, assume not joined
                not_joined.append(channel)
        ...
        try:
            ...
            logger.info(f"✅ Video {message_id} unlocked for user {user.id}")
